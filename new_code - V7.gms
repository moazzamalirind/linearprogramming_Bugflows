...
HighFlow_Hrs                  Number of hours in a day with High Flow /13/
LowFlow_Hrs                   Number of hours in a day with low Flow /11/
Rate_Highflow                 Per Hour rate for High flow hydropower output ($ per MWH)/60/
Rate_Lowflow                  Per Hour rate for High flow hydropower output ($ per MWH)/25/
...
hydropower_Objective          Maximize the overall monthly hydropower revenue ($)
...
hp1                           Total Monthly Hydropower Revenue generated from High flow ($)
hp2                           Total Monthly Hydropower Revenue generated from low flow ($
...
EQ12_highhydro               Total monthly Hydropower Revenue generated from High flow ($)
EQ13_lowhydro                Total monthly Hydropower Revenue generated from low flow ($)
Hydro_calculation            Total monthly Hydropower Revenue generated ($)
...
EQ1__ResMassBal(d)..         storage(d) =e= initstorage$(ord(d)eq 1)+ storage(d-1)$(ord(d)gt 1)+ (inflow(d)*conver)-Highvol(d)-Lowvol(d)-evap ;
...
EQ8a_Highflowvol(d,p)..      Highvol(d)=e= release(d,p)$(ord(p) eq 1)*factor_foracftperHr*HighFlow_Hrs;
...
EQ8b_Lowflowvol(d,p)..       Lowvol(d)=e= release(d,p)$(ord(p) eq 2)*factor_foracftperHr*LowFlow_Hrs;
...
EQ12_highhydro..             hp1=e= sum((d,p),release(d,p)$(ord(p) eq 1)*Density_Water*Elev_Head *Efficiency*factor_HptoKWH*KWHtoMWH_factor*Rate_Highflow*HighFlow_Hrs);
...
EQ13_lowhydro..              hp2=e=sum((d,p),release(d,p)$(ord(p) eq 2)*Density_Water*Elev_Head *Efficiency*factor_HptoKWH*KWHtoMWH_factor*Rate_Lowflow*LowFlow_Hrs );
...
Hydro_calculation..          hydropower_Objective=e= hp1+ hp2;
