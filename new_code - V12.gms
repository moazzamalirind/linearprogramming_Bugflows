...
Efficiency                    Efficieny of power turbines at GCD /0.8/
...
CombineObjective               Combine objective function
...
EQ8__Mayrel..                sum(d,sum(p,FlowVol(d,p)))=e=1284000;
...
EQ11_Hydropeakingindex(f)$(ord(f) eq 1)..     ObjectiveVal(f)=e= standarddev/Avgrelease;
*EQ 11 is calculating hydropeaking index value for the whole month.
EQ12_EnergyGen(d,p)..                         Energy_Gen(d,p)=e= release(d,p)*Density_Water*Elev_Head *Efficiency*factor_HptoKWH*KWHtoMWH_factor*Duration(p);
...
Execute_Unload "Bug_simulation_Energy_WAPAVALUE(cv0.21).gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Bug_simulation_Energy_WAPAVALUE(cv0.21).gdx"
