...
*EQ10_function(f)$(ord(f) eq 1)..             ObjectiveVal(f)=e= sum((d,p),(-1/2)*(arctan(((release(d,p)-MinRelease)-0.65)/0.001)/(pi/2)+1)+1);
...
*TotMonth_volume= Vol_monthlyrelease(tot_vol);
   ...
   SOLVE ExtremePt USING nlp maximize CombineObjective;
