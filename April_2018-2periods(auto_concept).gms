...
Ptotal(p,d,tot_vol)                     To check sum of the both periods equals to 24
...
Display d,inflow, initstorage, maxstorage, minstorage, maxRel, minRel, evap, p, Scen;
*===============================================
SCALAR
...
starttime(p)                  Start Time for sub-period(0-23)
...
EQ6a_minstartTime(p)         Starting Time constraint for the sub-period
EQ6b_maxstartTime(p)         Ending Time constraint for the sub-period
EQ6c_IncreaseStartTime(p)    Condition for the 2 sub-period
EQ6d_PeriodDuration(p)       Calculating the duration for sub-periods
EQ6e_DurationLastPeriod(p)   Calculating the duration for last sub-periods
EQ7_FlowVolume(d,p)          volume of water released per time step (acre-ft)
EQ8__Monthtlyrel             Constraining Total monthly volume of water released in "April" as per WAPA information(acre-ft)
...
EQ12a_EnergyGen_Max(d,p)     Maximum Energy Generation Limit of the Glen Caynon Dam(MW)for Periods
...
*Defining constraints for start and end time of sub-periods
EQ6a_minstartTime(p)..       starttime(p)=g=0;
EQ6b_maxstartTime(p)..       starttime(p)=l=23;

*Require the StartTime for subsequent periods to be greater than the start time for the prior period
EQ6c_IncreaseStartTime(p)$(ord(p) lt card(p))..   starttime(p+1)=g=starttime(p);

*calculating the period duration
EQ6d_PeriodDuration(p)$(ord(p) lt card(p))..  Duration(p)=e= starttime(p+1) - starttime(p);
*Duration for periods 1,2, ... P-1
EQ6e_DurationLastPeriod(p)$(ord(p) eq card(p)).. Duration(p)=e= starttime(p++1) + 23 -starttime(p);
*Duration for last period P (wraps around 24)
...
* Equation 9 is calculating the monlthy average release from the reservior. (Mathematical details of RHS: First summing daily two aggregated values and dividing by 24(hours in day)-for average- and then summing values for all days and dividing by total number of days i.e: 30 in April.
...
EQ12a_EnergyGen_Max_m(d,p,tot_vol)     Maximum Energy Generation Limit of the Glen Caynon Dam(MW)for Periods
...
*  Only consider the current objective
   ...
   Ptotal(p,d,tot_vol)= Duration.L(p);
...
MODEL ExtremePt_minus_simulation Find extreme points by using  NLP without simulation part /EQ1__ResMassBal,EQ2__reqpowerstorage,EQ3__maxstor,EQ4__MaxR,EQ5__MinR,EQ6_Energyrate,EQ6a_minstartTime,EQ6b_maxstartTime,EQ6c_IncreaseStartTime,EQ6d_PeriodDuration,EQ6e_DurationLastPeriod,EQ7_FlowVolume,EQ8__Monthtlyrel,EQ9_Avgrelease,EQ10_Standarddev, EQ11_Hydropeakingindex, EQ12_EnergyGen,EQ12a_EnergyGen_Max,EQ13_EnergyRevenue,EQ15_CombinedObjectives/;
...
MODEL ObjAsConstraint__minus_simulation Single-objective model with other objectives constrained /EQ1__ResMassBal,EQ2__reqpowerstorage,EQ3__maxstor,EQ4__MaxR,EQ5__MinR,EQ6_Energyrate,EQ6a_minstartTime,EQ6b_maxstartTime,EQ6c_IncreaseStartTime,EQ6d_PeriodDuration,EQ6e_DurationLastPeriod,EQ7_FlowVolume,EQ8__Monthtlyrel,EQ9_Avgrelease,EQ10_Standarddev, EQ11_Hydropeakingindex, EQ12_EnergyGen,EQ12a_EnergyGen_Max,EQ13_EnergyRevenue,EQ15_CombinedObjectives,ObjAsCon/;
...
* Dump all input data and results to a GAMS gdx file
Execute_Unload "Apri1l8_2periods_auto.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Apri1l8_2periods_auto.gdx"
