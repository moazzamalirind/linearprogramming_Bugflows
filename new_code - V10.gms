...
*Energy_Revenue                Maximize the overall monthly hydropower revenue($)
Hydro
BugIndex
...
*EQ8a_Highflowvol(d,p)$(ord(p) eq 1)..      Highvol(d)=e= release(d,p)*factor_foracftperHr*HighFlow_Hrs;
...
*EQ8b_Lowflowvol(d,p)$(ord(p) eq 2)..       Lowvol(d)=e= release(d,p)*factor_foracftperHr*LowFlow_Hrs;
...
MODEL ExtremePt Find an extreme point of the NLP
                     /EQ11_Hydropeakingindex,EQ12_EnergyRevenue,EQ7_FlowVolume,EQ1__ResMassBal,EQ4__MaxR/
...
*The objective as constraint is greater or less than the level set for that objective
ObjAsCon(f)$(1 - FtoUse(f))..        BugIndex=e=FLevel(f);
...
SOLVE ExtremePt USING NLP  MAXIMIZING Hydro;
...
FLevel("BugIndex")= 0.609;

SOLVE ObjAsConstraint USING NLP MAXIMIZING Hydro;


DISPLAY release.L,storage.L,BugIndex.L,FlowVol.L, Hydro.L;

* Dump all input data and results to a GAMS gdx file
Execute_Unload "Bug_trial.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Bug_trial.gdx"
