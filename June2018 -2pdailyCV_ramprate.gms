...
EQ7_Ramprate(d,p)            Constraining the daily fluction rate between the timesteps(cfs)
EQ7a_Rampratenext(d,p)       Constraining the daily fluction rate between the last timestep of current day and next timestep for next day(cfs)
EQ7b_FlowVolume(d,p)         volume of water released per time step (acre-ft)
...
EQ7_Ramprate(d,p)..          release(d,"pHigh")-release(d,"pLow")=l=Daily_Ramprate;
EQ7a_Rampratenext(d,p)$(ord(d) lt Numdays)..     release(d,"pHigh")-release(d+1,"pLow")=l=Daily_Ramprate;
EQ7b_FlowVolume(d,p)..       ReleaseVol(d,p) =e= release(d,p)*factor_foracftperHr*Duration(p);
...
EQ13a_EnergyGen_Max(d,p)..                    Energy_Gen(d,p)=l= 1320*Duration(p);
...
EQ7_Ramprate_m(d,p,tot_vol)            Constraining the daily fluction rate between the timesteps(cfs)
EQ7a_Rampratenext_m(d,p,tot_vol)       Constraining the daily fluction rate between the last timestep of current day and next timestep for next day(cfs)
EQ7b_FlowVolume_m(d,p,tot_vol)         volume of water released per time step (acre-ft)
...
EQ7_Ramprate_m2(d,p,tot_vol)            Constraining the daily fluction rate between the timesteps(cfs)
EQ7a_Rampratenext_m2(d,p,tot_vol)       Constraining the daily fluction rate between the last timestep of current day and next timestep for next day(cfs)
EQ7b_FlowVolume_m2(d,p,tot_vol)         volume of water released per time step (acre-ft)
...
EQ13a_EnergyGen_Max_m2(d,p,tot_vol)     Maximum Energy Generation Limit of the Glen Caynon Dam(MW)for each Period
...
EQ7_Ramprate_m(d,p,tot_vol)=EQ7_Ramprate.m(d,p);
EQ7a_Rampratenext_m(d,p,tot_vol)= EQ7a_Rampratenext.m(d,p)$(ord(d) lt Numdays);
EQ7b_FlowVolume_m(d,p,tot_vol)= EQ7b_FlowVolume.m(d,p);
...
MODEL ExtremePt_minus_simulation Find extreme points by using  NLP without simulation part /EQ1__ResMassBal,EQ2__reqpowerstorage,EQ3__maxstor,EQ4__MaxR,EQ5__MinR,EQ6_Energyrate,EQ7_Ramprate,EQ7a_Rampratenext,EQ7b_FlowVolume,EQ8__Monthtlyrel,EQ9_Avgrelease,EQ10_Standarddev,EQ11_daliyCV,EQ12_Hydropeakingindex, EQ13_EnergyGen,EQ13a_EnergyGen_Max,EQ14_EnergyRevenue,EQ15_CombinedObjectives/;
...
EQ7_Ramprate_m2(d,p,tot_vol)=EQ7_Ramprate.m(d,p);
EQ7a_Rampratenext_m2(d,p,tot_vol)= EQ7a_Rampratenext.m(d,p)$(ord(d) lt Numdays);
EQ7b_FlowVolume_m2(d,p,tot_vol)= EQ7b_FlowVolume.m(d,p);
...
EQ13a_EnergyGen_Max_m2(d,p,tot_vol)= EQ13a_EnergyGen_Max.m(d,p);
...
MODEL ObjAsConstraint__minus_simulation Single-objective model with other objectives constrained /EQ1__ResMassBal,EQ2__reqpowerstorage,EQ3__maxstor,EQ4__MaxR,EQ5__MinR,EQ6_Energyrate,EQ7_Ramprate,EQ7a_Rampratenext,EQ7b_FlowVolume,EQ8__Monthtlyrel,EQ9_Avgrelease,EQ10_Standarddev,EQ11_daliyCV,EQ12_Hydropeakingindex, EQ13_EnergyGen,EQ13a_EnergyGen_Max,EQ14_EnergyRevenue,EQ15_CombinedObjectives,ObjAsCon/;
...
Execute_Unload "June18_0to8_dailycvlimit.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls June18_0to8_dailycvlimit.gdx"
