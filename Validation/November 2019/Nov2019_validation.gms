...
release(p)                       Average release observed during each period (p) over the month (cfs) /pLow 9851.72, pHigh 11420.64/
...
Steady_Release                   Steady release value of the hydrograph /9851.72/
**Steady release is for weekends and here pLow= steady release (model assumption).

initstorage                           Initial reservoir i.e. storage observed on october 31 2019 (acre-ft)
...
EnergyRate(p)                         Energy prices on weekdays ($ per MWH) /pLow 48.3390, pHigh 62.1358/
weekendRate(p)                        Energy prices on weekends ($ per MWH) /pLow 48.3390, pHigh 48.3390/
Duration(p)                           Duration of period (hours)
Steady_Days                           The number of steady low flow days (observed during November 2019)/9/
;
...
Execute_Unload "valid_Nov19.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls valid_Nov19.gdx"
