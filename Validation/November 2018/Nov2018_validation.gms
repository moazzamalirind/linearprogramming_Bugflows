...
release(p)                       Average release observed during each period (p) over the month (cfs) /pLow 9925.80, pHigh 12607.99/
...
Steady_Release                   Steady release value of the hydrograph /9925.80/
**Steady release is for weekends and here pLow= steady release (model assumption).

initstorage                           Initial reservoir storage on November 1ST 2018 (acre-ft)
...
EnergyRate(p)                         Energy prices on weekdays ($ per MWH) /pLow 45.6895, pHigh 61.9794/
weekendRate(p)                        Energy prices on weekends ($ per MWH) /pLow 45.6895, pHigh 45.6895/
Duration(p)                           Duration of period (hours)
Steady_Days                           The number of steady low flow days (observed during Nov 2018)/8/
;
...
Avail_Water                    Total Water available in the system during the month (November 2018)
...
Execute_Unload "valid_Nov18.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls valid_Nov18.gdx"
