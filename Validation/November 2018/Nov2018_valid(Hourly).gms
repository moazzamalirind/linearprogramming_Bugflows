$ontext
Title Optimization model for Glen Canyon Dam releases to favor Bugs population. (November 2018)
...
initstorage                  Initial reservoir storage.. Storage observed on 31 october 2018 (acre-ft)
...
observed_release(d,p)        observed release at November 2018 averaged over hourly timesteps (cfs)
...
*===================================================
$CALL GDXXRW.EXE input=Input_Nov2018.xlsx output= Nov18_Valid(Hr).gdx  par=observed_release rng=Hourly_Rel!A1:Y32 par=EnergyRate rng=Rates!A1 Rdim=1  set=d rng=day!A1 Rdim=1  par=Inflow rng=inflow!A1 Rdim=1  par=initstorage rng=initstorage!A1 Rdim=0  par=maxstorage rng=maxstorage!A1 Rdim=0   par=minstorage rng=minstorage!A1 Rdim=0  par=maxRel rng=maxRel!A4 Rdim=0 par=minRel rng=minRel!A1 Rdim=0  par=evap rng=evap!A1 Rdim=0

*Write the input Data into a GDX file
$GDXIN Nov18_Valid(Hr).gdx
...
EQ7_FlowVolume(d,p)          volume of water released per timestep (acre-ft)
EQ7a_TotVolume               volume of water released per month (acre-ft)
EQ8_EnergyGen(d,p)           Amount of energy generated per timestep (MWH)
...
EQ11_ReleaseSim(d,p)         Setting release values as predefined for simulation(cfs)
...
EQ11_ReleaseSim(d,p)..                 release(d,p)=e=observed_release(d,p);
...
Execute_Unload "Nov18_Valid(Hr).gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Nov18_Valid(Hr).gdx"
