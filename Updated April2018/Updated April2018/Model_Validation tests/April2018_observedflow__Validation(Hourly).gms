...
*===================================================
$CALL GDXXRW.EXE input=Input_April_2018_validation.xlsx output= Validation_April2018(Hourly).gdx   par=observed_release rng=Observed_release2018(Hr)!A1:Y32 par=EnergyRate rng=Rates!A1 Rdim=1   set=d rng=day!A1 Rdim=1  par=Inflow rng=inflow!A1 Rdim=1  par=initstorage rng=initstorage!A1 Rdim=0  par=maxstorage rng=maxstorage!A1 Rdim=0   par=minstorage rng=minstorage!A1 Rdim=0  par=maxRel rng=maxRel!A1 Rdim=0 par=minRel rng=minRel!A1 Rdim=0  par=evap rng=evap!A1 Rdim=0

*Write the input Data into a GDX file
$GDXIN Validation_April2018(Hourly).gdx

*set=d par=Observed_release2018 rng=pHigh!A1 Rdim=1 rng=plow!A1 Rdim=1
...
Display d,inflow, initstorage, maxstorage, minstorage, maxRel, minRel, evap,p,observed_release,EnergyRate;
*Levels,Scen,
*===============================================
SCALAR
...
EQ7_FlowVolume(d,p)..        ReleaseVol(d,p) =e= release(d,p)*factor_foracftperHr;
EQ8__Mayrel..                sum(d,sum(p,ReleaseVol(d,p)))=e=Vol_monthlyrelease;
*EQ8b_  constraining the overall monthly released volume..
EQ9_Avgrelease..             Avgrelease=e= sum(d,sum(p,release(d,p))/24)/Numdays;
...
EQ12_EnergyGen(d,p)..                         Energy_Gen(d,p)=e= release(d,p)*0.03715;
...
Execute_Unload "Validation_April2018(Hourly).gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Validation_April2018(Hourly).gdx"
