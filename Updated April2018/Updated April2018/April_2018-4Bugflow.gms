...
*Setting Thursday, Friday, Saturday and Sunday to 1
Use_Sim(d,p) $((ord(d)>= 1  and ord(d)<= 1)
                    OR(ord(d)>= 5 and ord(d)<= 8)
                   OR (ord(d)>=12 and ord(d)<=15)
                  OR (ord(d)>=19 and ord(d)<=22)
                  OR (ord(d)>=26 and ord(d)<=29)) = 1;
*Set weekend flows to 8000 cfs.
...
EQ12a_EnergyGen_Max(d,p)     Maximum Energy Generation Limit of the Glen Caynon Dam(MW)for High Period
EQ12b_EnergyGen_Max(d,p)     Maximum Energy Generation Limit of the Glen Caynon Dam(MW) for Low Period
...
EQ12a_EnergyGen_Max(d,p)$(ord(p)eq 1)..                    Energy_Gen(d,p)=l= 1320*Duration("pLow");
EQ12b_EnergyGen_Max(d,p)$(ord(p)eq 2)..                    Energy_Gen(d,p)=l= 1320*Duration("pHigh");
...
EQ12a_EnergyGen_Max_m(d,p,tot_vol)     Maximum Energy Generation Limit of the Glen Caynon Dam(MW)for High Period
EQ12b_EnergyGen_Max_m(d,p,tot_vol)     Maximum Energy Generation Limit of the Glen Caynon Dam(MW) for Low Period)
...
EQ12a_EnergyGen_Max_m2(d,p,tot_vol)     Maximum Energy Generation Limit of the Glen Caynon Dam(MW)for High Period
EQ12b_EnergyGen_Max_m2(d,p,tot_vol)     Maximum Energy Generation Limit of the Glen Caynon Dam(MW) for Low Period
...
EQ12a_EnergyGen_Max_m(d,p,tot_vol)= EQ12a_EnergyGen_Max.m(d,p)$(ord(p)eq 1);
EQ12b_EnergyGen_Max_m(d,p,tot_vol)= EQ12b_EnergyGen_Max.m(d,p)$(ord(p)eq 2);
...
EQ12a_EnergyGen_Max_m2(d,p,tot_vol)= EQ12a_EnergyGen_Max.m(d,p)$(ord(p)eq 1);
EQ12b_EnergyGen_Max_m2(d,p,tot_vol)= EQ12b_EnergyGen_Max.m(d,p)$(ord(p)eq 2);
...
* Dump all input data and results to a GAMS gdx file
Execute_Unload "Apri1l8_4bugflowdays.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Apri1l8_4bugflowdays.gdx"
