...
Use_Sim(d,p) = 0;
Rel_vals(d,p)= 0;
*Setting Friday, Saturday and Sunday to 1
Use_Sim(d,p) $((ord(d)>= 1  and ord(d)<= 1)
                    OR(ord(d)>= 6 and ord(d)<= 8)
                   OR (ord(d)>=13 and ord(d)<=15)
                  OR (ord(d)>=20 and ord(d)<=22)
                  OR (ord(d)>=27 and ord(d)<=29)) = 1;
...
* Dump all input data and results to a GAMS gdx file
Execute_Unload "Apri1l8_3bugflowdays.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Apri1l8_3bugflowdays.gdx"
