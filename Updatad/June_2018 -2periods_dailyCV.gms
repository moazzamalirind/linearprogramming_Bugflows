...
*volume(tot_vol)                         Disclosing the exact volume released
*volume2(tot_vol)                        Disclosing the exact volume released
...
$CALL GDXXRW.EXE input=Input_June_2018.xls output= Junel8_dailycv00.gdx set=d rng=day!A1 Rdim=1  par=Inflow rng=inflow!A1 Rdim=1  par=initstorage rng=initstorage!A1 Rdim=0  par=maxstorage rng=maxstorage!A1 Rdim=0   par=minstorage rng=minstorage!A1 Rdim=0  par=maxRel rng=maxRel!A1 Rdim=0 par=minRel rng=minRel!A1 Rdim=0  par=evap rng=evap!A1 Rdim=0
*Note please go back to original file with original name.

*Write the input Data into a GDX file
$GDXIN Junel8_dailycv00.gdx
...
EQ7b_Rampup_ratenext(d,p)$(ord(d) lt Numdays)..     release(d+1,"pHigh")-release(d,"pLow")=l=Daily_Ramprate;
EQ7c_Rampdown_ratenext(d,p)$(ord(d) lt Numdays)..   release(d+1,"pHigh")-release(d,"pLow")=g= -1*Daily_Ramprate;
...
Execute_Unload "Junel8_dailycv00.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Junel8_dailycv00.gdx"
