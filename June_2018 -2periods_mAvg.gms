...
EQ9_Avgrelease(d)$(ord(d) lt Numdays) ..            Avgrelease(d)=e= (sum(p,release(d,p)*Duration(p))+sum(p,release(d+1,p)*Duration(p)))/48;
...
EQ10_Standarddev(d)$(ord(d) lt Numdays)..           standarddev(d)=e= sqrt((sum(p,Duration(p)*power((release(d,p)-Avgrelease(d)),2))+ sum(p,Duration(p)*power((release(d+1,p)-Avgrelease(d)),2)))/36);
...
EQ11_daliyCV(d)$(ord(d) lt Numdays)..                cv(d)=e= standarddev(d)/Avgrelease(d);
...
EQ12_Hydropeakingindex(f)$(ord(f) = 1 )..     ObjectiveVal(f)=e= Obj_dir(f)*(sum(d $ (ord(d)< Numdays),cv(d))/(Numdays-1));
...
Execute_Unload "Junel8_0to8_mavgcv.gdx";
* Dump the gdx file to an Excel workbook
Execute "gdx2xls Junel8_0to8_mavgcv.gdx"


*For Baron Solver in case of any issue with number of error itterations.
*MaxExecError = 100000;                                                 7\k
*option sys12 = 1;
